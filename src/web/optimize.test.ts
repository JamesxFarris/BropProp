...
//
// There were five tests here for `candidatesFor`, asserting that a leg's
// direction came from the book consensus where three books priced a market and
// from the projection otherwise. Both of those signals have since been graded
// and both are dead — the projection at AUC 0.495 and -9.5% ROI, the consensus
// at 74-73 over 147 series, p = 1.00 — so the function they tested is gone and
// `buildEntries` prices a leg at the book's own devigged number instead. The
// tests that matter now are the `marketCandidates` ones below.
