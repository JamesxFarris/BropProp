...
/**
 * Migrations run on every boot (`npm start` migrates, then schedules), so they
 * must be applied exactly once and never re-run.
 *
...
try {
  ...
  // One-time adoption path: a database built before this ledger existed already
  // has the schema but no record of it, and replaying 001 against it fails.
  // If the schema is present and the ledger is empty, treat what's on disk now
  // as the baseline. A genuinely fresh database has no `prop` table and falls
  // through to a normal first run.
  if (applied.size === 0) {
    const { rows } = await client.query<{ exists: boolean }>(`SELECT to_regclass('public.prop') IS NOT NULL AS exists`);
    if (rows[0]?.exists) {
